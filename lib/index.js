'use strict';var _typeof=typeof Symbol==="function"&&typeof(typeof Symbol==='function'?Symbol.iterator:'@@iterator')==="symbol"?function(obj){return typeof obj;}:function(obj){return obj&&typeof Symbol==="function"&&obj.constructor===Symbol&&obj!==(typeof Symbol==='function'?Symbol.prototype:'@@prototype')?"symbol":typeof obj;};var _createClass=function(){function defineProperties(target,props){for(var i=0;i<props.length;i++){var descriptor=props[i];descriptor.enumerable=descriptor.enumerable||false;descriptor.configurable=true;if("value"in descriptor)descriptor.writable=true;Object.defineProperty(target,descriptor.key,descriptor);}}return function(Constructor,protoProps,staticProps){if(protoProps)defineProperties(Constructor.prototype,protoProps);if(staticProps)defineProperties(Constructor,staticProps);return Constructor;};}();var _reactNative=require('react-native');function _classCallCheck(instance,Constructor){if(!(instance instanceof Constructor)){throw new TypeError("Cannot call a class as a function");}}




var RNTimer=_reactNative.NativeModules.RNTimer;var
...
function Timer(){var _this=this;_classCallCheck(this,Timer);this.timeouts=new Map();this.intervals=new Map();this.immediates=new Map();this.animationFrames=new Map();this.contextTimers=new WeakMap();this.timeDiff=0;
if(__DEV__&&_reactNative.Platform.OS=='android'){
var js_now_time=new Date().getTime();
js_now_time=""+js_now_time;

RNTimer.getTime(js_now_time,function(systemTime){
var sysTime=parseInt(systemTime.diff);
_this.timeDiff=sysTime<0?0:sysTime;
});
}
}_createClass(Timer,[{key:'setTimeout',value:function setTimeout()
...
name,fn,interval){var _this2=this;
this.clearTimeout(name);
this.timeouts.set(name,setTimeout(function(){
fn();
_this2.clearTimeout(name);
...
_this3.contextTimers.get(ctx).clearTimeout(timeout);
});
}else{var
timeout=args[0];
...
_this4.contextTimers.get(ctx).clearInterval(interval);
});
}else{var
interval=args[0];
...
_this6.contextTimers.get(ctx).clearImmediate(immediate);
});
}else{var
immediate=args[0];
...
_this8.contextTimers.get(ctx).cancelAnimationFrame(animationFrame);
});
}else{var
